...
no_of_servers = 100 #these server means only worker servers, not includes centralized server
...
def fennelLikeScoringFunctionRepartition_dummy(currNode, server):
	...
	score_second_term = pow(totalServerNode, gamma_repartition - 1)
	score_second_term = alpha_repartition*(gamma_repartition/2)*score
	score_val = serverNeighborNo - score_second_term
	return score_val
...
def ifProfitableRepartitionThisEdge(srcNode, destNode):
	...
	totalCommunicationCostIfThisRepartitionNotPerf = 0 
	totalCommunicationCostIfThisRepartitionPerf = 0 
	migrationCost = 0 
